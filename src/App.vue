  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">总结周期</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">侧重维度</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">个人成长复盘与里程碑沉淀指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 个人成长履历与里程碑总结专家');
...
const metricsList = [
  {
    "key": "milestoneClarity",
    "label": "里程碑节点清晰度"
  },
  {
    "key": "growthReflectionDepth",
    "label": "成长复盘反思深度"
  },
  {
    "key": "skillAccumulationValue",
    "label": "技能沉淀价值分"
  },
  {
...
const guideRules = [
  {
    "title": "用事实与数据说话",
    "advice": "总结成长时不仅记录感受，更要记录完成的项目、读过的书、提升的收入等具体成果",
    "avoid": "切忌只写感性情绪而缺少实际沉淀"
  },
  {
    "title": "深度复盘踩坑经验",
    "advice": "最宝贵的成长往往来自失败经历，深刻剖析原因并总结防坑 SOP",
    "avoid": "切勿只歌功颂德而回避真正的失误"
  },
  {
    "title": "形成闭环远景",
    "advice": "复盘的终点是指导未来，将反思转化为明确的下一阶段行动计划",
    "avoid": "避免总结完后束之高阁不付诸行动"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[LVCHENG_SCORES\][\s\S]*?\[\/LVCHENG_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[LVCHENG_SCORES\](.*?)\[\/LVCHENG_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('lvcheng_history_records');
...
const saveHistory = () => {
  localStorage.setItem('lvcheng_history_records', JSON.stringify(historyList.value));
};
